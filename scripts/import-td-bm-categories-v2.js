...
/**
 * Import Categories for TD BM (Tech Delivery Business Management) - Neon Compatible
 * 
...
async function promptForBusinessGroup(businessGroups) {
  ...
  return new Promise((resolve) => {
    rl.question('Select Business Group number (or press Enter for first one): ', (answer) => {
      rl.close();
...
async function importCategoriesByBusinessGroup(excelFilePath) {
  ...
  try {
    ...
    // Process imports (Neon doesn't support transactions, so we do sequential inserts)
    ...
    for (const [catName, catData] of categoriesMap.entries()) {
      ...
      const existingCat = await sql`
        ...
        WHERE name = ${catName} 
        AND business_unit_group_id = ${selectedBG.id}
      `;
      ...
      for (const subcat of catData.subcategories) {
        ...
        const existingMapping = await sql`
          ...
          WHERE target_business_group_id = ${selectedBG.id}
          AND category_id = ${categoryId}
        ...
        if (existingMapping.length === 0) {
          await sql`
            ...
            VALUES (${selectedBG.id}, ${selectedBG.id}, ${categoryId}, ${subcategoryId}, ${subcat.estimatedDuration})
          `;
          totalMappings++;
        }
      }
...
if (!excelFilePath) {
  console.log('Usage: node scripts/import-td-bm-categories-v2.js <excel-file-path>');
  console.log('');
  console.log('Example:');
  console.log('  node scripts/import-td-bm-categories-v2.js categories.xlsx');
  console.log('  node scripts/import-td-bm-categories-v2.js "/mnt/c/Users/User/Downloads/categories.xlsx"');
  console.log('');
