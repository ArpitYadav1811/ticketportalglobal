...
export default function AdminSidebar({ activeSection, onSectionChange, isSuperAdmin }: AdminSidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`${isCollapsed ? "w-16" : "w-64"} bg-card/95 backdrop-blur-md border-r border-border/50 h-full overflow-y-auto fixed lg:static z-40 transition-all duration-300 shadow-xl lg:shadow-none ${
      ...
      >
        <div className="p-3 border-b-2 border-border/50 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent backdrop-blur-sm relative overflow-hidden group">
          ...
          <div className="relative z-10 flex items-center justify-between">
            ...
            {isCollapsed && (
              <div className="w-8 h-8 bg-gradient-to-br from-primary via-primary/90 to-primary/80 rounded-lg flex items-center justify-center shadow-lg shadow-primary/30 mx-auto">
                <Settings className="w-4 h-4 text-primary-foreground drop-shadow-sm" />
              </div>
            )}
            <div className="flex items-center gap-1">
              <button
                ...
                className="hidden lg:flex p-1.5 hover:bg-muted/80 rounded-md transition-all duration-300 hover:scale-110 hover:shadow-sm"
          ...
          return (
            <div key={section.id} className="space-y-0.5">
              <div className="relative group/section">
                ...
                {isCollapsed && (
                  <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 z-50 opacity-0 group-hover/section:opacity-100 transition-opacity duration-300 pointer-events-none">
                    <div className="bg-gray-900 dark:bg-gray-800 text-white text-xs font-semibold px-2.5 py-1.5 rounded-lg shadow-xl border border-gray-700 whitespace-nowrap">
                      {section.label}
                ...
                <button
                  onClick={() => {
                    if (isCollapsed) {
                      // If collapsed, clicking should expand and show items
                  ...
                  className={`group relative w-full flex items-center ${isCollapsed ? "justify-center" : "justify-between"} px-3 py-2 rounded-lg text-xs font-bold transition-all duration-300 overflow-hidden ${
                    hasActiveItem
                ...
                <div className={`flex items-center ${isCollapsed ? "justify-center" : "gap-2"} relative z-10`}>
              ...
              {/* Expanded items when collapsed (shows on hover) */}
              {isExpanded && isCollapsed && (
                <div className="absolute left-full ml-2 top-0 z-50 opacity-0 group-hover/section:opacity-100 transition-opacity duration-300 pointer-events-none group-hover/section:pointer-events-auto">
                  <div className="bg-card/95 backdrop-blur-md border-2 border-border/50 rounded-lg shadow-2xl p-2 min-w-[200px] space-y-1">
                    {section.items.map((item) => {
                      const isActive = item.id === activeSection
                      return (
                        <button
                          key={item.id}
                          onClick={() => handleItemClick(item.id)}
                          className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md text-xs transition-all duration-300 ${
                            isActive
                              ? "bg-gradient-to-r from-primary via-primary/95 to-primary/90 text-primary-foreground font-bold"
